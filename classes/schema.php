...
class Schema {
  // constructs PHP data types from CSV files
  ...
  public static function get_col($index,$table) {
    $result = [];
    foreach($table as $row) { $result[] = $row[intval($index)]; }
    return $result;
  }

  public static function get_rows_from_cols($cols) {
    $rows = [];
    $row = [];
    $row_count = count($cols[0]);
